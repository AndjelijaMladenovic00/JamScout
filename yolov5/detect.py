...
def run(
        ...
        source=ROOT / 'data/images/test2017',  # file/dir/URL/glob/screen/0(webcam)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            annotator.box_label([5,10,25,25],label=f'Cars counted {len(track_vehicles)}', color = (0,255,0))  
            # Save results (image with detections)
            if save_img:
                if dataset.mode == 'image':
                    if len(det) and (2 in det[:,5].unique() or 3 in det[:,5] or 7 in det[:,5] or 5 in det[:,5]):
